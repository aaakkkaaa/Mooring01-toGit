...
namespace Obi
{
    public class BurstSolverImpl : ISolverImpl
    {
        ...
        List<IBurstConstraintsImpl>[] constraints;
        ...
        public BurstSolverImpl(ObiSolver solver)
        {
            ...
            constraints = new List<IBurstConstraintsImpl>[Oni.ConstraintTypeCount];
            constraints[(int)Oni.ConstraintType.Tether] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.Volume] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.Chain] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.Bending] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.Distance] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.ShapeMatching] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.BendTwist] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.StretchShear] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.Pin] = new List<IBurstConstraintsImpl>() { new BurstPinConstraints(this) };
            constraints[(int)Oni.ConstraintType.ParticleCollision] = new List<IBurstConstraintsImpl>() { new BurstParticleCollisionConstraints(this) };
            constraints[(int)Oni.ConstraintType.Density] = new List<IBurstConstraintsImpl>() { new BurstDensityConstraints(this) };
            constraints[(int)Oni.ConstraintType.Collision] = new List<IBurstConstraintsImpl>() { new BurstColliderCollisionConstraints(this) };
            constraints[(int)Oni.ConstraintType.Skin] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.Aerodynamics] = new List<IBurstConstraintsImpl>();
            constraints[(int)Oni.ConstraintType.Stitch] = new List<IBurstConstraintsImpl>() { new BurstStitchConstraints(this) };
            constraints[(int)Oni.ConstraintType.ParticleFriction] = new List<IBurstConstraintsImpl>() { new BurstParticleFrictionConstraints(this) };
            constraints[(int)Oni.ConstraintType.Friction] = new List<IBurstConstraintsImpl>() { new BurstColliderFrictionConstraints(this) };

            var c = constraints[(int)Oni.ConstraintType.Collision][0] as BurstColliderCollisionConstraints;
            c.CreateConstraintsBatch();

            var f = constraints[(int)Oni.ConstraintType.Friction][0] as BurstColliderFrictionConstraints;
        ...
        {
            IConstraintsImpl<IDistanceConstraintsBatchImpl> group = new BurstDistanceConstraints(this);
            constraints[(int)Oni.ConstraintType.Distance].Add((IBurstConstraintsImpl)group);
            return group;
        }
        ...
        {
            IConstraintsImpl<IVolumeConstraintsBatchImpl> group = new BurstVolumeConstraints(this);
            constraints[(int)Oni.ConstraintType.Volume].Add((IBurstConstraintsImpl)group);
            return group;
        }

        public IConstraintsImpl<IAerodynamicConstraintsBatchImpl> CreateAerodynamicConstraints()
        {
            IConstraintsImpl<IAerodynamicConstraintsBatchImpl> group = new BurstAerodynamicConstraints(this);
            constraints[(int)Oni.ConstraintType.Aerodynamics].Add((IBurstConstraintsImpl)group);
            return group;
        }

        public IConstraintsImpl<IStretchShearConstraintsBatchImpl> CreateStretchShearConstraints()
        {
            IConstraintsImpl<IStretchShearConstraintsBatchImpl> group = new BurstStretchShearConstraints(this);
            constraints[(int)Oni.ConstraintType.StretchShear].Add((IBurstConstraintsImpl)group);
            return group;
        }

        public IConstraintsImpl<IBendTwistConstraintsBatchImpl> CreateBendTwistConstraints()
        {
            IConstraintsImpl<IBendTwistConstraintsBatchImpl> group = new BurstBendTwistConstraints(this);
            constraints[(int)Oni.ConstraintType.BendTwist].Add((IBurstConstraintsImpl)group);
            return group;
        }

        public IConstraintsImpl<IChainConstraintsBatchImpl> CreateChainConstraints()
        {
            IConstraintsImpl<IChainConstraintsBatchImpl> group = new BurstChainConstraints(this);
            constraints[(int)Oni.ConstraintType.Chain].Add((IBurstConstraintsImpl)group);
            return group;
        }

        public IConstraintsImpl<ITetherConstraintsBatchImpl> CreateTetherConstraints()
        {
            IConstraintsImpl<ITetherConstraintsBatchImpl> group = new BurstTetherConstraints(this);
            constraints[(int)Oni.ConstraintType.Tether].Add((IBurstConstraintsImpl)group);
            return group;
        }
        ...
        public IConstraintsImpl<IStitchConstraintsBatchImpl> CreateStitchConstraints()
        {
            ...
            var cnst = constraints[(int)Oni.ConstraintType.Stitch];
            if (cnst.Count == 0)
                cnst.Add(new BurstStitchConstraints(this));
            return cnst[0] as IConstraintsImpl<IStitchConstraintsBatchImpl>;
        }

        public IConstraintsImpl<ISkinConstraintsBatchImpl> CreateSkinConstraints()
        {
            IConstraintsImpl<ISkinConstraintsBatchImpl> group = new BurstSkinConstraints(this);
            constraints[(int)Oni.ConstraintType.Skin].Add((IBurstConstraintsImpl)group);
            return group;
        }

        public IConstraintsImpl<IShapeMatchingConstraintsBatchImpl> CreateShapeMatchingConstraints()
        {
            IConstraintsImpl<IShapeMatchingConstraintsBatchImpl> group = new BurstShapeMatchingConstraints(this);
            constraints[(int)Oni.ConstraintType.ShapeMatching].Add((IBurstConstraintsImpl)group);
            return group;
        }
        ...
        {
            // singleton constraints:
            var cnst = constraints[(int)Oni.ConstraintType.Pin];
            if (cnst.Count == 0)
                cnst.Add(new BurstPinConstraints(this));
            return cnst[0] as IConstraintsImpl<IPinConstraintsBatchImpl>;
        }
        ...
        {
            var d = constraints[(int)Oni.ConstraintType.Density][0] as BurstDensityConstraints;
        ...
        protected JobHandle GenerateContacts(JobHandle inputDeps, float deltaTime)
        {
                ...
                densityParameters.enabled)
            {
                ...
                var pc = constraints[(int)Oni.ConstraintType.ParticleCollision][0] as BurstParticleCollisionConstraints;
                pc.batches.Clear();
                var pf = constraints[(int)Oni.ConstraintType.ParticleFriction][0] as BurstParticleFrictionConstraints;
                pf.batches.Clear();
                ...
                var dc = constraints[(int)Oni.ConstraintType.Density][0] as BurstDensityConstraints;
                dc.batches.Clear();
        ...
        {
            // Apply aerodynamics
            JobHandle aerodynamicsHandle = ProjectConstraints(new JobHandle(), Oni.ConstraintType.Aerodynamics, substepTime);
        ...
        private JobHandle InitializeConstraints(JobHandle inputDeps, Oni.ConstraintType type, float deltaTime)
        {
            ...
            if (group != null && group.Count > 0)
            {
                ...
                for (int i = 0; i < group.Count; ++i)
                    deps[i] = group[i].Initialize(inputDeps, deltaTime);
        ...
        private JobHandle ApplyConstraints(JobHandle inputDeps, float deltaTime)
        {
            ...
            for (int i = 0; i < Oni.ConstraintTypeCount; ++i)
            {
                ...
                if (parameters.enabled)
                {
                    ...
                    inputDeps = InitializeConstraints(inputDeps, (Oni.ConstraintType)i, deltaTime);
                }
            }
                ...
                for (int j = 0; j < Oni.ConstraintTypeCount; ++j)
                {
                    if (j != (int)Oni.ConstraintType.Aerodynamics)
                    {
                        ...
                        if (parameters.enabled && i % padding[j] == 0)
                            inputDeps = ProjectConstraints(inputDeps, (Oni.ConstraintType)j, deltaTime);
                    }
                }
            }
            ...
            for (int i = 0; i < Oni.ConstraintTypeCount; ++i)
            {
                if (i != (int)Oni.ConstraintType.Aerodynamics)
                {
                    var parameters = m_Solver.GetConstraintParameters((Oni.ConstraintType)i);
                    if (parameters.enabled && parameters.iterations > 0)
                        inputDeps = ProjectConstraints(inputDeps, (Oni.ConstraintType)i, deltaTime);
                }
            }
            ...
            if (param.enabled && param.iterations > 0)
                inputDeps = ProjectConstraints(inputDeps, Oni.ConstraintType.ParticleCollision, deltaTime);
            param = m_Solver.GetConstraintParameters(Oni.ConstraintType.Collision);
            if (param.enabled && param.iterations > 0)
                inputDeps = ProjectConstraints(inputDeps, Oni.ConstraintType.Collision, deltaTime);
