...
namespace NWH.DWP2.WaterData
{
    ...
    public abstract class WaterDataProvider : MonoBehaviour
    {
        ...
        public virtual void Awake()
        {
            ...
            {
                Debug.LogError("WaterDataProvider requires a Collider with 'Is Trigger' ticked to be present on the same GameObject.");
                return;
            }

            if (!_triggerCollider.isTrigger)
            {
                Debug.LogError("WaterDataProvider Collider has to have 'Is Trigger' ticked. " + gameObject.name);
                return;
            }
        }
