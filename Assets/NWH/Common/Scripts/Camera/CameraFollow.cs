...
namespace NWH.Common.Cameras
{
    ...
    public class CameraFollow : VehicleCamera
    {
        ...
        private void LateUpdate()
        {
            ...
            if (!firstFrame)
            {
                targetForward = Vector3.SmoothDamp(prevTargetForward, target.transform.forward,
                                                   ref targetForwardVelocity,
                                                   smoothing);
            }
            else
            {
            ...
            transform.LookAt(target.transform.position + Vector3.up * targetUpOffset +
                             target.transform.forward * targetForwardOffset);
        }
