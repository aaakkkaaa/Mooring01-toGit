...
namespace NWH.Common.CoM
{
    ...
    public class VariableCenterOfMass : MonoBehaviour, IMassAffector
    {
        ...
        public Vector3 CalculateInertiaTensor(Vector3 dimensions)
        {
            ...
            (
                (dimensions.y + dimensions.z) * 0.1f * totalMass,
                (dimensions.z + dimensions.x) * 0.1f * totalMass,
                (dimensions.x + dimensions.y) * 0.1f * totalMass
            );
            ...
            for(int i = 1; i < affectors.Length; i++) // Skip first (this)
            {
                ...
                if (affector.GetTransform().gameObject.activeInHierarchy)
                {
                    float c = affector.GetMass();
                    Vector3 affectorLocalPos = transform.InverseTransformPoint(affector.GetTransform().position);
                    affectorInertiaSum.x += Vector3.ProjectOnPlane(affectorLocalPos, Vector3.right).magnitude * c;
                    affectorInertiaSum.y += Vector3.ProjectOnPlane(affectorLocalPos, Vector3.up).magnitude * c;
                    affectorInertiaSum.z += Vector3.ProjectOnPlane(affectorLocalPos, Vector3.forward).magnitude * c;
                }
            }

            return Vector3.Scale(inertiaTensor + affectorInertiaSum, inertiaScale);
        }
